...
if.end146:                                        ; preds = %if.then123, %if.else138, %if.then93, %if.else108
...
sw.epilog:                                        ; preds = %for.body, %sw.default, %sw.bb94, %sw.bb81, %sw.bb48, %sw.bb37, %sw.bb20, %sw.bb
...
entry:
  ...
  br i1 %cmp218, label %for.body.lr.ph, label %for.cond.cleanup
...
for.cond.cleanup:                                 ; preds = %if.end, %entry
  %carry1.0.lcssa = phi i32 [ %b, %entry ], [ %xor5, %if.end ]
  %carry2.0.lcssa = phi i32 [ %c, %entry ], [ %mul7, %if.end ]
  %carry0.0.lcssa = phi i32 [ %a, %entry ], [ %xor, %if.end ]
  %result.0.lcssa = phi i64 [ 0, %entry ], [ %result.1, %if.end ]
  %mul44 = mul nsw i32 %mul, %c
  %conv45 = sext i32 %mul44 to i64
  %add46 = add nsw i64 %result.0.lcssa, %conv45
  %cmp52227 = icmp sgt i32 %n, 0
  br i1 %cmp52227, label %for.body54.lr.ph, label %for.cond.cleanup53
...
for.cond.cleanup53:                               ; preds = %for.cond.cleanup53.loopexit, %for.cond.cleanup
  %carry3.0.lcssa = phi i32 [ %carry1.0.lcssa, %for.cond.cleanup ], [ %7, %for.cond.cleanup53.loopexit ]
  %result.2.lcssa = phi i64 [ %add46, %for.cond.cleanup ], [ %add105, %for.cond.cleanup53.loopexit ]
  %sub = add nsw i32 %n, -1
...
for.body54:                                       ; preds = %for.body54.lr.ph, %if.end90
...
cleanup111:                                       ; preds = %if.then23, %if.end74, %if.then
...
if.end:                                           ; preds = %if.then, %entry
  ...
  br i1 %process_row2, label %if.then94, label %if.end145
